...
import { subscribeToUser, updateVisibility } from '../services/userService';
...
export default function NearbyPage() {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToUser(user.uid, (dbUser) => {
      setEnabled(Boolean(dbUser?.visibilityEnabled));
  ...
  if (loadingVisibility) {
    return <div className="p-6 text-slate-500">Loading nearby settings...</div>;
  }
        ...
        right={
          <button
            ...
            className={`rounded-2xl px-4 py-2 text-sm font-medium ${
              enabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-700'
            }`}
          >
